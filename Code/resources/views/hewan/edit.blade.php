...
<div class="max-w-5xl mx-auto py-8 px-4">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <h1 class="text-3xl font-bold text-blue-900">Edit Hewan</h1>
            <p class="text-sm text-gray-600">Perbarui informasi hewan peliharaan.</p>
        </div>
    ...
    <form action="{{ route('admin.hewan.update', $hewan) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-2xl shadow border border-gray-100 space-y-4">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label class="block font-semibold text-gray-700">Nama</label>
            <input type="text" name="nama" value="{{ old('nama', $hewan->nama) }}" class="border-2 border-blue-200 p-3 w-full rounded-xl bg-white text-blue-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none">
            @error('nama')
        ...
        <div class="mb-4">
            ...
            <div class="relative">
                <select name="jenis" class="border-2 border-blue-200 p-3 w-full rounded-full bg-white text-blue-900 font-semibold pr-10 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    ...
                    <option value="kucing" {{ old('jenis', $hewan->jenis) === 'kucing' ? 'selected' : '' }}>Kucing</option>
                    <option value="anjing" {{ old('jenis', $hewan->jenis) === 'anjing' ? 'selected' : '' }}>Anjing</option>
                </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="ras" value="{{ old('ras', $hewan->ras) }}" class="border-2 border-blue-200 p-3 w-full rounded-xl bg-white text-blue-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none">
            @error('ras')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="text" name="warna" value="{{ old('warna', $hewan->warna) }}" class="border-2 border-blue-200 p-3 w-full rounded-xl bg-white text-blue-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Contoh: Cokelat, Hitam Putih">
            @error('warna')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="text" name="kepribadian" value="{{ old('kepribadian', $hewan->kepribadian) }}" class="border-2 border-blue-200 p-3 w-full rounded-xl bg-white text-blue-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Contoh: Ramah, Aktif, Tenang">
            @error('kepribadian')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <textarea name="deskripsi" class="border-2 border-blue-200 p-3 w-full rounded-xl bg-white text-blue-900 focus:ring-2 focus:ring-blue-500 focus:outline-none" rows="4">{{ old('deskripsi', $hewan->deskripsi) }}</textarea>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="usia" value="{{ old('usia', $hewan->usia) }}" class="border-2 border-blue-200 p-3 w-full rounded-xl bg-white text-blue-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Contoh: 2 years / 6 months">
            @error('usia')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <div class="relative">
                <select name="gender" class="border-2 border-blue-200 p-3 w-full rounded-full bg-white text-blue-900 font-semibold pr-10 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    ...
                    <option value="jantan" {{ old('gender', $hewan->gender) === 'jantan' ? 'selected' : '' }}>Jantan</option>
                    <option value="betina" {{ old('gender', $hewan->gender) === 'betina' ? 'selected' : '' }}>Betina</option>
                </select>
                <svg class="w-4 h-4 text-blue-700 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z" clip-rule="evenodd" />
                </svg>
        ...
        <div class="flex gap-3 items-center pt-2">
            <button type="submit" class="bg-blue-700 text-white px-5 py-2 rounded-full shadow hover:bg-blue-800 transition">Simpan Perubahan</button>
